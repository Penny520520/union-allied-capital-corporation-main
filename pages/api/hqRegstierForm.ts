...
import {
  hqaddMemberToMailingListAsync,
  HQIMailingListMember,
} from '~/server/utils';
...
export default async function contactAsync(
  ...
  try {
    ...
    const result = await hqaddMemberToMailingListAsync({
...
>(req: RequestType, res: ResponseType): void | HQIMailingListMember {
